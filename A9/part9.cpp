...
//Xinyun Li     21884782
/// this skeleton contains the sun, the earth roatating around it and a simple camera navigation.
/// It is enough to implement the current assignment but we highly recommend to use your old code.
 ...
 * texture coordinates and tangents
 */
struct Vertex {
...
{ 
	...
	// Create Corona Geometry
 	p = glm::vec3(0,0,0);
 	c = glm::vec4(0.9,0.9,0,1.0);
 	vertexdata.push_back(Vertex(p,c));
 	indices.push_back(index++);
 	float r = 1;
 	c = glm::vec4(0.9,0.9,0,0.0);
 	for (int i=0;i<=10;++i){
 	   float arg = M_PI * (float)i/5.0;
 	...
 	//vertexdata.push_back(Vertex(glm::vec3(0,0,0),glm::vec3(1,0,0)));
...
// creates a sphere with normals and tangents
void createSphere()
{
	float r=1;
	int slices = 25;
...
void display()
{
	...
	M = glm::translate(M, 200.f,0.f,0.f);
	glm::mat4 M_saturn = M;
	...
	M = glm::scale(M,50.f,50.f,50.f);
	CoronaShader.bindUniforms(M,V,P,LightSource,Color,0,t);
	
	glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
	glEnable(GL_BLEND);
	
	glBindVertexArray(geometryCorona.vao);
	glDrawElements(GL_TRIANGLE_FAN,geometryCorona.numIndices,GL_UNSIGNED_SHORT,0);
	glBindVertexArray(0);
...
void cleanUp() {
    ...
    glDeleteProgram(CoronaShader.Shader);
